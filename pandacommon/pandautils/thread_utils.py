...
class MapWithLockAndTimeout(dict):
    ...
    def __setitem__(self, item, value):
        with self.lock:
            dict.__setitem__(self, item, {"time_stamp": datetime.datetime.now(datetime.UTC), "data": value})
    ...
    def __contains__(self, item):
        with self.lock:
            try:
                ret = dict.__getitem__(self, item)
                if ret["time_stamp"] > datetime.datetime.now(datetime.UTC) - datetime.timedelta(minutes=self.timeout):
                    return True
            except Exception:
                pass
        return False
